                 ...
                 struct xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_LIBERA_ENCAP: {
            struct ofpact_libera_encap* libera = ofpact_get_LIBERA_ENCAP(a);
            ...
            //VLOG_ERR("kllaf do_xlate_actions sp : %hd dp : %hd sip : %d dip : %d ", libera->src_port, libera->dest_port, libera->src_ipv4, libera->dest_ipv4);
            ctx->xout->slow |= commit_odp_actions(&ctx->xin->flow, &ctx->base_flow, ctx->odp_actions, ctx->wc, ctx->xbridge->support.masked_set_action);
            nl_msg_put_unspec(ctx->odp_actions, OVS_ACTION_LIBERA_ENCAP, libera, sizeof(struct ofpact_libera_encap));
            break;
            }

        case OFPACT_LIBERA_DECAP:
            VLOG_ERR("kllaf xlate DECAP");

            struct ofpact_libera_decap* libera = ofpact_get_LIBERA_DECAP(a);
            ctx->xout->slow |= commit_odp_actions(&ctx->xin->flow, &ctx->base_flow, ctx->odp_actions, ctx->wc, ctx->xbridge->support.masked_set_action);
            nl_msg_put_unspec(ctx->odp_actions, OVS_ACTION_LIBERA_DECAP, libera, sizeof(struct ofpact_libera_decap));
            break;
        }
