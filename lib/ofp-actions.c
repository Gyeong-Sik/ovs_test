...
/* Actions similar to standard actions are listed with the standard actions. */
    ...
    /* OF1.0(30): struct libera_encap_action */
    LIBERA_RAW_ENCAP,

    /* OF1.0(31): struct libera_decap_action */
    LIBERA_RAW_DECAP,
...
ofpact_next_flattened(const struct ofpact *ofpact)
{
    ...
    case OFPACT_LIBERA_ENCAP:
    case OFPACT_LIBERA_DECAP:
...
OFP_ASSERT(sizeof(struct libera_encap_action)==32);

struct libera_decap_action{
    ovs_be16 type;
    ovs_be16 len;
    ovs_be32 pad;
};
OFP_ASSERT(sizeof(struct libera_decap_action)==8);




static enum ofperr decode_LIBERA_RAW_ENCAP(const struct libera_encap_action * pa, enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf * out){
    struct ofpact_libera_encap *libera = ofpbuf_put_zeros(out, 32);
    //libera = ofpact_put_raw(out, OFP10_VERSION, LIBERA_RAW_ENCAP, 0);
    libera->ofpact.type = 30;
    libera->ofpact.len = 32;
     
    libera->src_port = u16_to_ofp(ntohs(pa->src_port));
    libera->dest_port = u16_to_ofp(ntohs(pa->dest_port));
    libera->src_ipv4 = (pa->src_ipv4);
    libera->dest_ipv4 = (pa->dest_ipv4);
    libera->src_mac = pa->src_mac;
    libera->dest_mac = pa->dest_mac;

    /*
    VLOG_ERR("decode LIBERA_RAW_ENCAP : sp %hd dp %d", libera->src_port, libera->dest_port);
    VLOG_ERR("decode LIBERA_RAW_ENCAP : sip %hhd.%hhd.%hhd.%hhd dip %hhd.%hhd.%hhd.%hhd", (unsigned char)((libera->src_ipv4 >> 24) & 0xFF), (unsigned char)((libera->src_ipv4 >> 16) & 0xFF), (unsigned char)((libera->src_ipv4 >> 8) & 0xFF), (unsigned char)(libera->src_ipv4 & 0xFF),
        (unsigned char)((libera->dest_ipv4 >> 24) & 0xFF), (unsigned char)((libera->dest_ipv4 >> 16) & 0xFF), (unsigned char)((libera->dest_ipv4 >> 8) & 0xFF), (unsigned char)(libera->dest_ipv4 & 0xFF));
    VLOG_ERR("decode LIBERA_RAW_ENCAP : smac %hhd:%hhd:%hhd:%hhd:%hhd:%hhd dmac %hhd:%hhd:%hhd:%hhd:%hhd:%hhd",
        libera->src_mac.ea[5], libera->src_mac.ea[4],
        libera->src_mac.ea[3], libera->src_mac.ea[2],
        libera->src_mac.ea[1], libera->src_mac.ea[0],
        libera->dest_mac.ea[5], libera->dest_mac.ea[4],
        libera->dest_mac.ea[3], libera->dest_mac.ea[2],
        libera->dest_mac.ea[1], libera->dest_mac.ea[0]);
    */
      return 0;
}

static void encode_LIBERA_ENCAP(const struct ofpact_libera_encap* pa, enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf * out){
    struct libera_encap_action* libera;
    libera = ofpact_put_raw(out, 1, LIBERA_RAW_ENCAP, 0);
    libera->src_port = u16_to_ofp(ntohs(pa->src_port));
    libera->dest_port = u16_to_ofp(ntohs(pa->dest_port));
    libera->src_ipv4 = pa->src_ipv4;
    libera->dest_ipv4 = pa->dest_ipv4;
...
static char* OVS_WARN_UNUSED_RESULT parse_LIBERA_ENCAP(char* arg, struct ofpbuf* ofpacts, enum ofputil_protocol * usable_protocols OVS_UNUSED){
    return NULL;
}

static void format_LIBERA_ENCAP(const struct ofpact_libera_encap* libera, struct ds* s){
    ds_put_format(s, "libera_encap");
}


// these for decapsulation (yks)
static enum ofperr decode_LIBERA_RAW_DECAP(const struct libera_decap_action * pa, enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf * out){
    struct ofpact_libera_decap *libera = ofpbuf_put_zeros(out, 8);
    //kllaf
    libera->ofpact.type = 31;
    libera->ofpact.len = 8;
...
static void encode_LIBERA_DECAP(const struct ofpact_libera_decap* pa, enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf * out){
	ofpact_put_raw(out, 1, LIBERA_RAW_DECAP, 0);

}

static char* OVS_WARN_UNUSED_RESULT parse_LIBERA_DECAP(char* arg, struct ofpbuf* ofpacts, enum ofputil_protocol * usable_protocols OVS_UNUSED){
    return NULL;
}

static void format_LIBERA_DECAP(const struct ofpact_libera_decap* libera, struct ds* s){
    ds_put_format(s, "libera_decap");
}
...
//kllaf
//VLOG_WARN("kllaf ofpacts_decode type : %d, len : %d\n", action->type, action->len);
        }
...
get_ofpact_map(enum ofp_version version)
{
    ...
    static const struct ofpact_map of10[] = {
        ...
        { OFPACT_ENQUEUE, 11 },
        { OFPACT_LIBERA_ENCAP, 12 },
        { OFPACT_LIBERA_DECAP, 13 },
        { 0, -1 },
    };
                ...
                enum ofp_raw_action_type *raw, uint64_t *arg)
{
    ...
    *raw = *arg = 0;
    ...
    //if(oah->type != 0)
    //VLOG_WARN("kllaf pull_raw action : %s\n", action->name );
